...
// calculatePopupWindowPosition flag TrackPopupMenu[Ex] flags
const (
...
type ANIMATIONINFO struct {
	CbSize      uint32 //结构大小（以字节为单位）。 调用方必须将其设置为 sizeof(ANIMATIONINFO)。
	IMinAnimate int    //如果此成员为非零，则启用最小化和还原动画;否则会禁用它
}
...
var (
	...
	allowSetForegroundWindow     *windows.LazyProc
	attachThreadInput            *windows.LazyProc
	animateWindow                *windows.LazyProc
	arrangeIconicWindows         *windows.LazyProc
	anyPopup                     *windows.LazyProc
	beginDeferWindowPos          *windows.LazyProc
	beginPaint                   *windows.LazyProc
	bringWindowToTop             *windows.LazyProc
	calculatePopupWindowPosition *windows.LazyProc
	callMsgFilter                *windows.LazyProc
	callWindowProc               *windows.LazyProc
	cascadeWindows               *windows.LazyProc
	childWindowFromPointEx       *windows.LazyProc
...
func init() {
	...
	drawTextEx = user32.NewProc("DrawTextExW")
	...
	loadString = user32.NewProc("LoadStringW")
...
func AllowSetForegroundWindow(dwProcessId uint32) bool {
	if allowSetForegroundWindow == nil {
		allowSetForegroundWindow = user32.NewProc("AllowSetForegroundWindow")
	}
	ret, _, _ := syscall.SyscallN(adjustWindowRect.Addr(), uintptr(dwProcessId))
...
func AnyPopup() bool {
	...
	ret, _, _ := syscall.SyscallN(anyPopup.Addr())

	return ret != 0
}

func BeginDeferWindowPos(nNumWindows int32) HDWP {
	if beginDeferWindowPos == nil {
...
func CalculatePopupWindowPosition(anchorPoint *POINT, windowSize *SIZE, flag uint32, excludeRect *RECT, popupWindowPosition *RECT) bool {
	if calculatePopupWindowPosition == nil {
		calculatePopupWindowPosition = user32.NewProc("CalculatePopupWindowPosition")
	}
	ret, _, _ := syscall.SyscallN(calculatePopupWindowPosition.Addr(),
		uintptr(unsafe.Pointer(anchorPoint)), uintptr(unsafe.Pointer(windowSize)),
		uintptr(flag), uintptr(unsafe.Pointer(excludeRect)), uintptr(unsafe.Pointer(popupWindowPosition)))
	return ret != 0
}

//如果应用程序应进一步处理消息，则返回值为零。

// 如果应用程序不应进一步处理消息，则返回值为非零值。
func CallMsgFilter(msg *MSG, ncode int) bool {
	if callMsgFilter == nil {
		callMsgFilter = user32.NewProc("CallMsgFilterW")
	}
	ret, _, _ := syscall.SyscallN(callMsgFilter.Addr(),
		uintptr(unsafe.Pointer(msg)), uintptr(ncode))
	return ret != 0
}
func CallWindowProc(lpPrevWndFunc uintptr, hWnd HWND, Msg uint32, wParam, lParam uintptr) uintptr {
	if callWindowProc == nil {
...
func CascadeWindows(hwndParent HWND, wHow uint32, lpRect *RECT, cKids uint32, lpKids HWND) uintptr {
	if cascadeWindows == nil {
		cascadeWindows = user32.NewProc("CascadeWindows")
	}
	ret, _, _ := syscall.SyscallN(cascadeWindows.Addr(),
		uintptr(hwndParent),
		uintptr(wHow),
		uintptr(unsafe.Pointer(lpRect)),
		uintptr(cKids),
...
func ChildWindowFromPointEx(hwnd HWND, pt *POINT, flags uint32) HWND {
	if childWindowFromPointEx == nil {
		childWindowFromPointEx = user32.NewProc("ChildWindowFromPointEx")
	}
	ret, _, _ := syscall.SyscallN(childWindowFromPointEx.Addr(),
		uintptr(hwnd),
...
func CloseWindow(hwnd HWND) bool {
	...
	ret, _, _ := syscall.SyscallN(closeWindow.Addr(),
		uintptr(hwnd))
	return ret != 0
...
func ChangeWindowMessageFilterEx(hwnd HWND, msg uint32, action uint32, changeFilterStruct *CHANGEFILTERSTRUCT) bool {
	if changeWindowMessageFilterEx == nil {
		changeWindowMessageFilterEx = user32.NewProc("ChangeWindowMessageFilterEx")
	}
...
func SetForegroundWindow(hWnd HWND) bool {
	if setForegroundWindow == nil {
		setForegroundWindow = user32.NewProc("SetForegroundWindow")
	}
	ret, _, _ := syscall.SyscallN(setForegroundWindow.Addr(), uintptr(hWnd))
	return ret != 0
}
...
func ShowWindow(hWnd HWND, nCmdShow int32) bool {
	ret, _, _ := syscall.SyscallN(showWindow.Addr(), uintptr(hWnd), uintptr(nCmdShow))
	return ret != 0
}
